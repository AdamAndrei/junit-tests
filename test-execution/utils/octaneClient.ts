...
const getTestRunsInSuiteRun = async (suiteRunId: string): Promise<OctaneTestRun[]> => {
    let query;
    query = Query.field('parent_suite').equal(Query.field('id').equal(suiteRunId));
    const octaneResponse = await octane
        .get(Octane.entityTypes.manualRuns)
        .fields(
            'id',
            'parent_suite',
            'test{name}'
    ...
    if (octaneResponse.data[0] === undefined) {
        throw new Error(
            `Not found! Could not get test runs from Suite Run with id: ${suiteRunId}.`
        );
    }

    return <OctaneTestRun[]>octaneResponse.data
}
